...
class Document:
    def __init__(self, doc_id: str, title: str, content: str, category: str, version: str = "1.0.0", min_role: str = "Citizen"):
        self.doc_id = doc_id
        self.title = title
...
class KnowledgeBase:
    def __init__(self):
        self.docs: List[Document] = [
            ...
            Document(
                ...
                category="SOP",
                version="2.1.0",
                min_role="Government" # Restrict SOP to Government/Admin roles
            ),
            Document(
                doc_id="rule_1",
    ...
    def query(self, query: str, limit: int = 2, user_role: str = "Citizen") -> List[Dict[str, Any]]:
        # Retrieval validation: ensure query passes basic checks
        query_words = set(query.lower().split())
        matched = []
        user_level = ROLE_LEVELS.get(user_role, 1)

        for doc in self.docs:
            # 1. Document Access Control (check permission)
            doc_min_level = ROLE_LEVELS.get(doc.min_role, 1)
            if user_level < doc_min_level:
